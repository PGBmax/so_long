...
/*   Updated: 2026/04/13 15:22:59 by pboucher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handler(int n)
{
	...
	game = get_tgame();
	if (n == SIGINT || n == SIGQUIT)
	{
		ft_printf("\n\e[95m\tSIGNAL Detected: End of Program.\n\n\e[97m");
...
{
	mlx_loop_hook(game->mlx, (void (*))ft_hook, (void *)game);
...
int	main(int ac, char **av)
{
	...
	set_tgame(&game);
	mlx_init_hook(&game);
	mlx_set_icon(game.mlx, game.textures.digit[getpid() % 10]);
	mlx_loop(game.mlx);
	free_all(&game);
}
